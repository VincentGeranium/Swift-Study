/*:
 ...
 - 암시적 반환 표현
 */
// 함수는 클로저의 일종, 다시말해 함수는 이름이 있는 클로저
// 클로저는 이름 없는 함수와 같다고 생각하면 편한데 클로저가 함수보다 더 큰 범위임을 잊으면 안된다
...
// 위의 예시의 method가 클로저 즉, 마지막 전달인자가 클로저
...
// 암시적 반환 표현
// 클로저가 반환하는 값이 있다면
// 클로저의 마지막 줄의 결과값은 암시적으로 반환값을 취급한다
result = calculate(a: 10, b: 10) {
    $0 + $1
}
